...
class TextSegmentAgent(BaseAgent):

    def __init__(self, apikey, model="deepseek-reasoner"):
        super().__init__()
        self.apikey = apikey
        self.model = model

    def open(self):
        pass  # Open any resources if needed

    def close(self):
        pass  # Close any resources if needed

    def __call__(self, 字幕列表, user_content=None, system_prompt=text_seg_system_prompt):
        return text_segment_openai_call(self.apikey, 字幕列表, self.model, system_prompt, user_content)
